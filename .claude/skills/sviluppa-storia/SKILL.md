...
5. **Replica in locale i controlli della pipeline CI, non solo un sottoinsieme**: leggi il workflow file (`.github/workflows/*.yml`) e rilancia esattamente gli step lì definiti (es. `npm run lint`, `npm run lint:style`, `npm run typecheck`, `npm run build`, e qualsiasi altro gate come Lighthouse) prima di passare a Fase 4. In particolare:
   - Usa `npm ci`, non `npm install`, per il passo di installazione: `npm install` riscrive e "ripara" silenziosamente un `package-lock.json` disallineato, mentre `npm ci` (quello che gira davvero in CI) fallisce con `EUSAGE` se il lockfile non è perfettamente sincronizzato con `package.json` — un disallineamento che `npm install` maschera senza dirtelo.
   - Se la versione locale di Node/npm diverge da quella dichiarata nel workflow (`actions/setup-node` → `node-version`), allineati con un version manager (es. `nvm use <versione>`) prima di lanciare `npm ci`: il controllo di sincronizzazione del lockfile di `npm ci` può comportarsi diversamente tra versioni npm, quindi un `npm ci` verde con una npm diversa da quella CI non garantisce che la pipeline passi (visto in pratica su questo repo: lockfile aggiornato con npm 11 locale, ma npm 10 della pipeline lo rifiutava per pacchetti opzionali mancanti).
   - Se rigeneri il lockfile per risolvere un disallineamento, ricontrolla `npm ci` da zero (rimuovendo `node_modules`) prima di considerarlo risolto.
6. Se durante l'esecuzione scopri che il piano approvato non regge (assunzione sbagliata, requisito mancante), fermati e torna dall'utente invece di deviare in silenzio — un piano approvato non è una licenza a improvvisare cambi di scope.
